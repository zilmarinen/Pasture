...
//
//  Created by Zack Brown on 06/08/2021.
//
...
struct BuildingWindow: Prop {
    ...
    func build(position: Vector) -> [Euclid.Polygon] {
        ...
        let windowTextureCoordinates = UVs(start: Vector(0.5, 0.125), end: Vector(1.0, 0.25))
        
        let offset = position + Vector(0, 0.05, 0)
        ...
        let inner = Mesh(shape(position: offset, uvs: borderTextureCoordinates, width: Constants.width - 0.1, depth: 0.07, height: Constants.height - 0.1))
        let window = Mesh(shape(position: offset, uvs: windowTextureCoordinates, width: Constants.width - 0.1, depth: 0.03, height: Constants.height - 0.1))
        
        return outer.subtract(inner).union(window).polygons
    }
